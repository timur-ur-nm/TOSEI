...
export default function useFilterCar(sortedCars, filter) {
  const sortedAndFilteredCars = sortedCars.filter((car) => {
    ...
    if (filter.car_vin && !car.car_vin?.toLowerCase().includes(filter.car_vin.toLowerCase())) return false
    if (filter.price && !car.price?.includes(filter.price)) return false
    if (filter.car_model_year && !String(car.car_model_year).includes(filter.car_model_year)) return false
    return true;
  });
